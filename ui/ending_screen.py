...
class EndingScreen:
    def __init__(self, screen, fonts, ending_data):
        ...
        # Safe font loading – INCREASED FONT SIZES
        self.title_font = self._safe_font(fonts, 'xlarge', 52)    # Was 36, now 52
        self.sub_font   = self._safe_font(fonts, 'large', 32)     # Was 24, now 32
        self.body_font  = self._safe_font(fonts, 'medium', 26)    # Was 18, now 26
        self.continue_font = self._safe_font(fonts, 'medium', 20)  # Was 16, now 20

        # Wrap body text with wider width for larger font
        self.body_lines = self._wrap_text(ending_data.get("body", ""), self.body_font, self.W - 120)
        ...
        # Wrap context lines with wider width
        wrapped = []
        for line in self.context_lines:
            if line.startswith("  ") or line.startswith("\n"):
                wrapped.append(line)
            else:
                wrapped.extend(self._wrap_text(line, self.body_font, self.W - 120))
        ...
        try:
            while not self.done:
                ...
                if subtitle:
                    ...
                    self.screen.blit(sub_surf, (self.W//2 - sub_surf.get_width()//2, 120))

                # Body - with adjusted Y position for larger text
                y = 180
                ...
                # Continue prompt - larger
                cont_surf = self.continue_font.render("Press SPACE or click to continue", True, (100, 100, 80))
                self.screen.blit(cont_surf, (self.W//2 - cont_surf.get_width()//2, self.H - 60))
